...
const DashboardData = () => {
  const { listings, rattingRatio } = useListingStore();
  const navigate = useNavigate();
  const [ratingRatio, setRatingRatio] = useState<number>(0);

  useEffect(()=>{
    const fetchData = async () => {
      const ratio = await rattingRatio();
      setRatingRatio(ratio);
    };
    fetchData();
  },[rattingRatio]);
    ...
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <div className="bg-white shadow-md rounded-lg p-4 flex flex-col items-center justify-center">
          ...
          <h2 className="txt-xl">{ratingRatio}</h2>
        </div>
        <div className="bg-white shadow-md rounded-lg p-4 flex flex-col items-center justify-center">
          ...
          <button
            ...
            className="btn btn-primary bg-red-500 border border-red-500"
          >
            Add a Listing
          </button>
        </div>
